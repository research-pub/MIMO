...
class BertAttentionDag(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, input_tensor_dag, output_attentions=False):
        self_output = self.self(input_tensor, attention_mask, output_attentions)
        self_output_dag = self.self_dag(input_tensor_dag, attention_mask, output_attentions)

        attention_output = self.output(self_output[0], input_tensor)
        attention_output_dag = self.output_dag(self_output_dag[0], input_tensor_dag)
        outputs = (attention_output, attention_output_dag,) + self_output[1:]  # add attentions if we output them
        return outputs
...
class BertLayerDag(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_dag, output_attentions=False):
        attention_output, attention_output_dag, outputs = \
            self.attention(hidden_states, attention_mask, hidden_states_dag, output_attentions)
...
class BertEncoderDag(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, dag_inputs,
                output_all_encoded_layers=True, output_attentions=False):
        all_encoder_layers = []
        all_encoder_dags = []
        all_attentions = () if output_attentions else None
        for layer_module in self.layer:
            hidden_states, dag_inputs, attentions = layer_module(hidden_states, attention_mask,
                                                                 dag_inputs, output_attentions)
            if output_attentions:
