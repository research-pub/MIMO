...
from dataset_mimo import FTDataset, collate_pt, load_data
from model import BertConfig, NextDxPrediction
...
def main():
    ...
    task = args.task + '_lr_' + str(args.learning_rate) + '_bs_' + str(args.train_batch_size)  + \
           '_e_' + str(args.num_train_epochs) + '_l_' + str(args.lamda) + '_tr_' + str(args.train_ratio)
    ...
    config_json = 'model/config_backup.json'
    # config_json = 'KEMCE/model/config_backup.json'
    copyfile(config_json, os.path.join(output_dir, 'config_backup.json'))
