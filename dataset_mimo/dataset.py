...
def group_data(sequences, labels, labels_current_visit, group=1):
    ...
    count_labels = []
    count_labels.extend(collections.Counter(all_labels).most_common())

    label_size = len(count_labels)
    ind = list(range(label_size))
    percentile = int(0.2 * label_size)

    percentile_inds  = ind[(group-1)*percentile:group*percentile]
    labels_set = {count_labels[i][0] for i in percentile_inds}

    new_labels = []
    new_seqs = []
    new_labels_cat1 = []
    for pi, patient in enumerate(labels):
        seq = sequences[pi]
        labels_c1 = labels_current_visit[pi]

        pat_labels = []
        pat_seqs = []
        pat_labels_cat1 = []
        for vi, labs in enumerate(patient):
            labs_set = set(labs)
            inters = labs_set.intersection(labels_set)
            n_inters = len(inters)
            if n_inters > 0:
                pat_labels.append(labs)
                # pat_labels.append(list(inters))
                pat_seqs.append(seq[vi])
                pat_labels_cat1.append(labels_c1[vi])
        n_pat_labels = len(pat_labels)
        if n_pat_labels > 0:
            new_labels.append(pat_labels)
            new_seqs.append(pat_seqs)
            new_labels_cat1.append(pat_labels_cat1)

    return new_seqs, new_labels, new_labels_cat1
